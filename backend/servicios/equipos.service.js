...
class EquiposService {
    async getAllEquipos(query) {
        ...
        sql += " ORDER BY e.ine ASC";

        const equipos = await db.all(sql, params);

        // Obtener especificaciones para todos los equipos en una sola consulta si es posible, 
        // o por cada equipo (para este volumen actual e.map es aceptable)
        const result = await Promise.all(equipos.map(async (e) => {
            const specs = await db.all("SELECT id, clave, valor FROM especificaciones WHERE equipo_id = ? ORDER BY id ASC", [e.id]);
