...
class IPAMService {
    ...
    async getNetworks() {
        const manualRedes = await db.all('SELECT * FROM redes ORDER BY nombre ASC');
        const specs = await this.getEquipoNetworkRows();
        const equipoMap = this.buildEquipoNetworkMap(specs);
        const detectedSegments = new Map();

        Object.values(equipoMap).forEach(equipo => {
            if (!equipo.ip) return;
        ...
        const allRedes = manualRedes.map(red => {
            try {
    ...
    async getNetworkDetails(redId) {
        ...
        const equipoNetworkMap = this.buildEquipoNetworkMap(allSpecs);
        const detectedGateways = Array.from(new Set(
            Object.values(equipoNetworkMap)
                .map(e => e.gateway)
                .filter(ip => ip && this.isIpInNetwork(ip, meta))
        ));
        ...
        const reserved = (await db.all('SELECT * FROM ips_reservadas WHERE red_id = ?', [redId]))
            .filter(row => this.isValidIp(row.ip) && this.isIpInNetwork(row.ip, meta));

        const occupiedByIp = new Map(occupied.map(row => [row.ip, row]));
        const reservedByIp = new Map(reserved.map(row => [row.ip, row]));
        ...
        for (let i = 0; i < displayLimit; i++) {
            ...
            const occ = occupiedByIp.get(currentIp);
            const res = reservedByIp.get(currentIp);
