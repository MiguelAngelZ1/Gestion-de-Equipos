...
if (!JWT_SECRET) {
    if (IS_PROD) {
        logger.fatal("JWT_SECRET no definido en producción. El sistema no es seguro.");
        process.exit(1);
    } else {
        logger.fatal("JWT_SECRET no definido en desarrollo. El servidor no puede iniciar sin una clave secreta.");
        process.exit(1);
    }
}
